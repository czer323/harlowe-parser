...
tags: [feature, parser, tokenizer, harlowe, typescript, chevrotain, vitest, fuzzing]
---

# Introduction

This implementation plan outlines the steps and requirements for developing a robust tokenizer and parser for the Harlowe programming language using TypeScript, Chevrotain, and vitest. The goal is to enable accurate, maintainable, and testable parsing of Harlowe syntax for downstream formatting, linting, or analysis tools, with a focus on strong static typing and advanced testing (including grammar-based fuzzing).
...
- **REQ-009**: All code must be covered by vitest unit tests, including edge cases, error handling, and grammar-based fuzzing
...
- **GUD-001**: Follow best practices for Chevrotain integration, TypeScript typing, and vitest test structure
...
5. Write comprehensive vitest tests for the lexer and parser (unit, edge, error, stateful, Unicode)
...
7. [ ] **tests/lexer.test.ts** — Vitest tests for the lexer. (Depends on lexer and types.)
8. [ ] **tests/parser.test.ts** — Vitest tests for the parser. (Depends on index, parser, and types.)
